...
이 문서는 실제 백엔드 없이 **프런트엔드 State만으로 동작하는 프로토타입/데모**의 구조를 팀원이 빠르게 파악할 수 있도록 정리한 것입니다.
...
|---|---|---|
| `admin` | `admin123` | 관리자 |
| `worker` | `worker123` | 작업자 (김도윤, `workerId: w1`) |
...
- **백엔드/DB 없음**: 모든 데이터는 `mockData.ts` 시드 값 + 브라우저 메모리 state. 새로고침하면 전부 초기화됩니다.
