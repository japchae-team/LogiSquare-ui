...
- **안전 이벤트 캡처 이미지는 이제 백엔드가 프록시합니다.** `captureUrl`이 더 이상 AI 서버 절대경로나 우리 도메인 상대경로가 아니라 `GET /api/safety/events/{eventId}/capture-image`(백엔드가 원본을 가져와 바이트로 직접 스트리밍, 10분 캐시)로 내려옵니다. 프론트는 `<img src={captureUrl}>`을 그대로 쓰고 있어서 이 변경에 코드 수정 없이 자동으로 맞춰졌습니다. 다만 AI 서버에 원본 파일이 이미 없어진 옛날 이벤트는 여전히 400(`"Failed to load capture image."` 등)이 나서 이미지가 안 뜰 수 있습니다 — 이건 프록시가 아니라 원본 데이터 자체가 없는 문제라 프론트에서 고칠 수 없습니다.
...
### 안전 관리 (양쪽 공통, 단 조작 권한·위반자 이름 노출은 역할별로 다름)
- "현재 발생 중인 위반" 카드 그리드(미해결 이벤트) + 5초짜리 실시간 알림 팝업(소리 포함) + 전체 이력 테이블. **조회 자체는 관리자·작업자 모두 가능**합니다. 행/카드를 클릭하면 상세 모달이 열립니다.
- **위반자 이름은 관리자만 전체를 볼 수 있습니다.** 작업자는 본인이 위반자로 지정된 건만 이름이 보이고, 다른 사람 건은 카드/목록/상세 모달 어디서나 이름이 가려집니다 (`SafetyPage.tsx`의 `visibleWorkerName`). 처리자(`resolvedByName`)는 이제 화면에 표시하지 않습니다.
- 상세 모달 공통 표시: CCTV 캡처 이미지, 보호장비 착용 여부, (조건부) 위반자, 메모.
